...
export const DEFAULT_SKILLS = [
  {
    ...
    content: [
      ...
      '- 加备注：只在任务处于 Backlog 或 HIL 时能加（In Progress 是只读的）。',
      ...
      '- 人加了备注后，agent 应把它当成新的输入，折进描述，再用 digest_key_points 标记已消化。'
    ].join('\n')
  },
  {
    ...
    content: [
      ...
      '- 动手前先消化：任务带 needsDigest 时，先把新备注折进描述，再用 digest_key_points 标记已消化。',
  ...
  },
  {
    ...
    content: [
      ...
      '',
      '【备注与返工】',
      '- 任务带 needsDigest 时先别开工：把给 agent 的备注折进描述，再 digest_key_points 标记已消化。',
