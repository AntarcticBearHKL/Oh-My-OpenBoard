...
vi.mock('../../src/modules/swimlane-controls.js', () => ({ syncSwimLaneControls: vi.fn() }));
...
test('the swim lane view applies the same ordering inside a cell', () => {
  mocks.settings = { swimLanesEnabled: true, swimLaneGroupBy: 'label' };
  mocks.tasks = [
    { id: 'cell-late', column: BACKLOG_COLUMN_ID, creationDate: '2026-01-02T00:00:00.000Z' },
    { id: 'cell-note', column: BACKLOG_COLUMN_ID, creationDate: '2026-01-03T00:00:00.000Z', needsDigest: true }
  ];

  mountBoardContainer();
  renderBoard();

  const ids = [...document.querySelectorAll(`.swimlane-cell[data-column="${BACKLOG_COLUMN_ID}"] .task`)]
    .map((element) => element.dataset.taskId);
  expect(ids).toEqual(['cell-note', 'cell-late']);
});
