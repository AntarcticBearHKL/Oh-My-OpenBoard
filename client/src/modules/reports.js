...
function buildBarChartOption({ labels, seriesList, granularity, legend, yName = 'Tasks' }) {
...
function computeVelocityRows(boards) {
...
function computeBurndownSeries(tasks, columns, days, iteration) {
...
function computeCycleDistribution(tasks, columns) {
...
function buildBurndownOption({ labels, remaining, ideal }) {
  const theme = getChartTheme();
  return {
    backgroundColor: 'transparent',
    textStyle: { color: theme.text },
